...
def charge(x, y, sign="+", label="", r=14, color=None):
    ...
    if label: s += f'<text x="{x}" y="{y+r+16}" text-anchor="middle" font-size="13" fill="{INK}">{label}</text>'
    return s

def dot(x, y, label="", r=4, color=None, dy=-8):
    c = color or INK
    s = f'<circle cx="{x}" cy="{y}" r="{r}" fill="{c}"/>'
    if label: s += f'<text x="{x+8}" y="{y+dy+8}" font-size="13" fill="{c}">{label}</text>'
    return s
...
def arrow(x1, y1, x2, y2, color=None, label="", w=2, lx=0, ly=-6, dash=""):
    ...
    if label: s += f'<text x="{(x1+x2)/2+lx}" y="{(y1+y2)/2+ly}" text-anchor="middle" font-size="13" fill="{c}">{label}</text>'
    return s
...
def text(x, y, t, size=14, color=None, anchor="start", bold=False):
    c = color or INK
    return f'<text x="{x}" y="{y}" font-size="{size}" fill="{c}" text-anchor="{anchor}"{" font-weight=\"700\"" if bold else ""}>{t}</text>'
