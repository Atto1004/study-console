...
(function(){
  ...
  var DAYS={
    ...
    "2026-09-03":{range:"학습지 (2) 기본 연산 · (3) 행렬식(2차·3차 사루스)",summary:"덧셈·실수배·곱셈·거듭제곱 → 행렬식 — 시험은 행렬식부터 나온다",
      ...
      exam:[],tasks:[{text:"학습지 Ex01~Ex04 재풀이(답 가리고) — 전치·대칭 판별, AB^t, A²=O, det=0의 x, 3차 사루스",due:"2026-09-22",src:"정리 9/1·9/3"}],links:[]},
    "2026-09-08":{range:"학습지 (4) 소행렬식·여인수 → (5) 역행렬 → (6) 연립방정식 → (7) 크래머",summary:"결석분 — 아토가 학습지 p.7~14를 직접 풀어둔 PDF로 정리(Ex05~Ex10 검산 전부 정답)",
      ...
      tasks:[{text:"시험 대비 재풀이: Ex06 여인수 전개(다른 행 기준으로도) · Ex8 3×3 역행렬 · Ex10 크래머 — 답 가리고 1회씩",due:"2026-09-22",src:"정리 9/8"},
             {text:"9/3 예고 강의자료실 연습문제(행렬·행렬식, 답안 포함) — LMS 토큰 연결 후 내려받기",due:"",src:"녹음 9/3 1:01:20"}],links:[]},
  ...
  var _boot=boot;
  boot=function(){ _boot(); var ch=false; try{ ch=!term().patchV36a; V36.patch(); }catch(e){ if(window.console) console.warn("V36 패치 오류", e); } if(ch){ try{ render(); }catch(e){} } };
