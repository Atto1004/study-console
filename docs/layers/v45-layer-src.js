...
(function(){
  ...
  V45.findGroup=function(){
    return V45.api("/api/conversations").then(function(r){ if(r.status===401) throw {kind:"unauth"}; if(!r.ok) throw {kind:"http",status:r.status}; return r.json(); })
      .then(function(list){ var arr=Array.isArray(list)?list:(list&&list.conversations)||[]; var g=arr.filter(function(c){return c.kind==="group";})[0]; if(!g) throw {kind:"nogroup"}; V45.groupId=g.id; return g.id; });
  };
  ...
  V45.renderMsgs=function(soft){
    ...
    box.innerHTML=all.length?all.map(function(m){ var role=m.role==="user"?"user":m.role==="otta"?"otta":"assistant"; var long=role==="assistant"&&m.text.length>V45.LIM.answerNote;
      ...
      return '<div class="v45-m '+role+(m.provisional?" prov":"")+'"><div class="v45-who">'+V45.roleName[role]+(m.provisional&&!m.done&&role!=="user"?' <i>…</i>':'')+'</div><div class="v45-b">'+md(body)+ctxHtml+'</div>'+(long?'<div class="v45-note">1,500자 이후는 오타 검증에 들어가지 않았습니다(서버 맥락 절단)</div>':'')+'</div>'; }).join("")
      :'<div class="v45-empty">덱을 보다가 막히면 여기서 물어보세요. 아톰이 설명하고 오타가 검증합니다.</div>';
  ...
  V45.ensure=function(){
    ...
    var h=$("#ntv .ntv-h"); var btn=document.createElement("button"); btn.className="btn xs v45-btn"; btn.id="v45Btn"; btn.textContent="질문"; btn.title="아톰·오타에게 질문"; btn.onclick=V45.toggle; h.appendChild(btn);
    ...
    var p=document.createElement("aside"); p.id="v45Panel"; p.setAttribute("aria-label","아톰·오타에게 질문");
    p.innerHTML='<div class="v45-h"><div class="v45-ht"><b>아톰 · 오타에게 질문</b><span class="v45-mut">아톰·오타 그룹 채팅과 같은 대화입니다</span></div><button class="btn xs" id="v45Close" aria-label="닫기">닫기</button></div>'+
