...
(function(){
  ...
  /* PC 인벤토리 조회: {rec:n,pre:n,...} · null = 인벤토리 없음/못 읽음 */
  V32.pcOf=function(cname,date){ return V32.LIB? (V32.LIBMAP[cname+"|"+date]||{}) : null; };
  ...
  mustHTML=function(sl,counts){
    ...
    MUST.forEach(function(m){
      ...
      if(r.legacy>0) bits.push('<span class="v32-legacy">미분류 '+r.legacy+'개</span>');
  ...
  /* logSheet 래핑 — 현재 회차를 기억(mustHTML 이 PC 인벤토리를 찾을 때), kind 선택지 보강, 미분류 첨부에 분류 셀렉트 */
  ...
  logSheet=function(cid,date){
    ...
    var box=$("#lqAtt",root); if(!box||!window.MutationObserver) return;
    var decorate=function(){
      $$(".att-cell",box).forEach(function(cell){
        ...
        sel.onchange=function(){
          var k=sel.value; if(k==="미분류") return;
          idb.get(id).then(function(a){ if(!a) return; a.kind=k; return idb.put(a); })
            .then(function(){ toast(k+"로 분류했습니다"); closeSheet(); logSheet(cid,date); });
        };
  ...
  V32.fillDots=function(scope){
    ...
    $$(".v32-dots",scope).forEach(function(el){
      ...
      idb.allForSafe(key).then(function(r){
        ...
        var counts={ok:r.ok,map:{}};
        if(r.ok) r.list.forEach(function(a){ if(a.tl) return; var k=(typeof a.kind==="string"&&a.kind)?a.kind:"_none"; counts.map[k]=(counts.map[k]||0)+1; });
  ...
  V32.renderGrid=function(){
    ...
    if(table){
      ...
      html='<div class="v32-wknav"><button class="btn sm" data-gw="-1">‹</button><b>'+esc(V32.weekLabel(w2))+(w2===cw?' <span class="chip">이번 주</span>':'')+'</b><button class="btn sm" data-gw="1">›</button></div>'+
        [1,2,3,4,5].map(function(d){ var o=dows2[d];
          return '<div class="v32-day'+(o.date===td?" today":"")+'"><div class="v32-dh">'+DAY[d]+' <small>'+esc(V32.md(o.date))+'</small>'+(o.hol?' <span class="v32-tag">휴강</span>':'')+'</div>'+
            (o.hol?'':(o.items.length?o.items.map(function(c){return V32.cellLine(c,o.date,td);}).join(""):'<div class="hint">수업 없음</div>'))+'</div>'; }).join("");
    }
  ...
  V32.renderLib=function(){
    ...
    var type=ui.libType||"rec", cs=courses(), byName={}; cs.forEach(function(c){ byName[c.name]=c; });
    ...
    if(type==="dev"){
      ...
      cs.forEach(function(c){ sessions(c.id).forEach(function(s){ jobs.push(idb.allForSafe(sessKey(c.id,s.date)).then(function(r){ if(r.ok) r.list.forEach(function(a){ if(!a.tl) out.push({c:c,date:s.date,a:a}); }); })); }); });
      Promise.all(jobs).then(function(){
        ...
        box.className=""; box.innerHTML= out.length? '<div class="card v32-lib">'+out.map(function(o){
          return '<div class="v32-li"><span class="v32-ld">'+esc(V32.abbr(o.c.name))+' '+esc(V32.md(o.date))+'</span><span class="v32-lf">'+esc(o.a.name||"파일")+' <span class="v32-tag'+(o.a.kind===V32.KIND_LEGACY?" warn":"")+'">'+esc(o.a.kind||"자료")+(o.a.kind===V32.KIND_LEGACY?" · 미분류":"")+'</span></span>'+
  ...
  css.textContent=
    ...
    '.sdot.pc{background:#2563EB}.v32-legacy{color:#B7791F;font-weight:600}.v32-reclass{margin-top:4px;font-size:11px;width:100%}'+
